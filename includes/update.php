...
function sp_notify_success($post, $meta) {
  if (sp_notify_on_success()) {
    ...
    wp_mail(
      ...
      "SharePress Success",
      "Successfully posted $link to $meta->profile_service_tag. Congratulations!"
    );
  }
}
...
function sp_notify_error($post, $meta, $error) {
  ...
  if ($post) {
    if (sp_notify_on_error()) { 
      ...
      $message = "SharePress Error: $error; while sending \"{$meta->text}\" to Facebook for post {$post->ID}\n\nTo retry, simply edit your post and save it again:\n{$link}";
      if ($code == 1611070) { 
